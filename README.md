...
6종(카테고리별 1편)을 한 배치로 올리는 흐름은 **두 명령**이다. 가운데에 사람 검수가
들어가므로 한 명령으로 합칠 수 없다.

```
py -3 tools\collect.py                         # ① 후보 수집 (1분)
py -3 tools\draft.py <url1> ... <url6> --check  # ② 초안 6편 + 자동 대조 (3분)

run_preview.bat                                #    검수 — !! · ?? 항목부터

py -3 tools\publish.py --apply --en            # ③ 발행 + 영어판 (1분)
git push
```

기계 시간 약 5분 + 사람 검수 5~10분 = **총 10~15분.**

검수를 자동화하지 않는 이유는 실측 오류율이 편당 5건이기 때문이다. 현 연준 의장을
`파월`로(→케빈 워시), 노무현 사위를 `곽상원`으로(→곽상언), `보완수사권`을
`보안수사권`으로 쓴 사례가 모두 사람이 읽어서 걸렸다.
...
py -3 tools\publish.py --apply --en    발행 + 영어판까지 한 번에
git push
```

`--en`을 붙이면 발행한 기사들의 영어판을 이어서 만든다. 편당 1~2천 토큰이다.

발행 후 **이 배치가 6개 카테고리를 어떻게 덮었는지** 알려준다. 빠진 카테고리와 중복을
짚어 주므로 '6종 1개씩'이 목표일 때 확인하면 된다.

```
발행 완료 6건. 인덱스 자동카드 11개.
  이번 배치: 정치, 경제, 암호화폐, 스포츠, 세계, 스타트업
```
...
바꾸려면 config.json 의 `telegram_categories` 를 고친다. 슬러그를 직접 지정해서
부르면 필터를 건너뛴다 — 사람이 이름을 대고 부른 것은 의도한 것으로 본다.
